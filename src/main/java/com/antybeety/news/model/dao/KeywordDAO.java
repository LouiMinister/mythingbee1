...
public class KeywordDAO {
    ...
    /*키워드 코드(전달인자)를 포함한 마지막 기사의 코드 찾기*/
    ...
    //키워드 삭제(이름으로 검색)
    public int deleteKeyword(String name) {
        KeywordMapper mapper = getMapper();
        if(name==null){
            return -1;
        }
        String code = searchCodeByName(name);
        if (code == null) {//이름에 해당하는 코드 없을경우 -1 반환
            return -1;
        }
        return mapper.deleteKeyword(code);
    }

    public int cutKeywordBtwArticle(String keCode, String arCode) {
        KeywordMapper mapper =getMapper();
    ...
    public int cutAllKeywordBtwArticle(String arCode){
        KeywordMapper mapper =getMapper();
